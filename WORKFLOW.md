...
- **"Yes" Selection**:
  - Saves current practitioner to `enrolledPractitioners` array
  - Clears existing practitioner-specific form fields
  - Maintains `practitionerEnrollmentType = 'Existing Practitioner'`
  - **LOOPS BACK** to Step 11 (existing-practitioner-open-cics)
  - Allows adding multiple existing practitioners sequentially

- **"No" Selection**:
  - Saves final practitioner to `enrolledPractitioners` array
...
- **"Yes" Selection**:
  ...
  - Maintains `practitionerEnrollmentType = 'New Practitioner'`
  - **LOOPS BACK** to Step 24 (create-enrollment)
  - Allows enrolling multiple new practitioners sequentially

- **"No" Selection**:
  - Saves final new practitioner to `enrolledPractitioners` array
  - Sets `practitionerEnrollmentType = 'Existing Practitioner'`
...
│  (Based on enrolledPractitioners array)  │
│                                      │
...
5. System asks: "Are there more practitioners to verify?"
   - **Yes** → Saves current practitioner to `enrolledPractitioners` array with appropriate status, clears form, loops back to Question 20
   - **No** → Saves final practitioner, proceeds to Questions 21-23

**Practitioner Statuses Assigned**:
- `practitionerAlreadyEnrolled = 'No'` → Status: **"Verified - Ready to Enroll"**
- `practitionerAlreadyEnrolled = 'Yes'` AND `practitionerInGroup = 'No'` → Status: **"Verified - Ready to Add to Group"**
- `practitionerAlreadyEnrolled = 'Yes'` AND `practitionerInGroup = 'Yes'` → **Duplicate Rejection**

#### Phase 2: Automatic Routing (After Questions 21-23)
After completing Questions 21-23, the system examines the `enrolledPractitioners` array and automatically routes to the appropriate workflow:
...
The workflows process practitioners from the `enrolledPractitioners` array ONE AT A TIME:
...
4. **Automatic Transitions**: System automatically transitions from new → existing practitioner workflows based on enrolledPractitioners array
...
**Features**:
- `enrolledPractitioners` array stores all completed practitioner records
...
- Checks `practitionerEnrollmentType` state variable
...
- Pages conditionally rendered based on `practitionerEnrollmentType`
...
**File Structure**:
```
AppData/Roaming/com.practitionerenrollment.walkthrough/
  └── PWO_12345/
...
**Detection Logic**:
```javascript
// Check if practitioner already in enrolledPractitioners array
const isDuplicate = enrolledPractitioners.some(p =>
  p.data.npiNumber === currentPractitionerNPI
);
...
2. System detects match in `enrolledPractitioners`
...
- Added NPI/practitioner identifier comparison in enrolledPractitioners array
...
const [enrolledPractitioners, setEnrolledPractitioners] = useState([]);
const [currentEnrollingPractitionerIndex, setCurrentEnrollingPractitionerIndex] = useState(0);
const [currentAddingPractitionerIndex, setCurrentAddingPractitionerIndex] = useState(0);

// Workflow Control
const [practitionerEnrollmentType, setPractitionerEnrollmentType] = useState('New Practitioner');
...
// Manual Tracking
const [manualPractitionerData, setManualPractitionerData] = useState([]);
...
```javascript
{
  ...
  conditionalDisplay: (formData) => formData.practitionerEnrollmentType === 'New Practitioner',
...
```json
{
  ...
  "formData": {
    ...
    "practitionerAlreadyEnrolled": "no",
  ...
  "practitionerEnrollmentType": "New Practitioner",
  "currentEnrollingPractitionerIndex": 0,
  "currentAddingPractitionerIndex": 0
}
```
...
```
%APPDATA%\com.practitionerenrollment.walkthrough\
├── PWO_12345\
